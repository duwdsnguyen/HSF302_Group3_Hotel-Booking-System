    ...
    public CommandLineRunner init(RoleRepository roleRepository, UserRepository userRepository,PasswordEncoder passwordEncoder) {
        return args -> {
            ...
            if (!userRepository.existsUserByEmail("admin123@gmail.com")) {
                User user = new User("Nguyễn Văn Admin", "admin123@gmail.com", passwordEncoder.encode("admin123"), "123456789");
                userRepository.save(user);
            }

            if (!userRepository.existsUserByEmail("recept123@gmail.com")) {
                User user = new User("Nguyễn Văn Lễ Tân", "recept123@gmail.com", passwordEncoder.encode("recept123"), "0914505521");
                userRepository.save(user);
            }

            if (!userRepository.existsUserByEmail("guest123@gmail.com")) {
                User user = new User("Nguyễn Văn Khách", "guest123@gmail.com", passwordEncoder.encode("guest123"), "0914505521");
                userRepository.save(user);
            }


            userRepository.findUserByEmail("admin123@gmail.com").ifPresent((user) -> {
                user.setRoles(adRole);
                userRepository.save(user);
            });

            userRepository.findUserByEmail("guest123@gmail.com").ifPresent((user) -> {
                user.setRoles(gRole);
                userRepository.save(user);
            });

            userRepository.findUserByEmail("recept123@gmail.com").ifPresent((user) -> {
